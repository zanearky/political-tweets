...
load("~/political-tweets/data/data_constructed/election_fraud.RData")
load("~/political-tweets/data/data_constructed/democracy.RData")
```
...
# Data and Methods
  ...
  An engagement weight is assigned to each tweet, that classifies it as being in either the 99th, 95th, 90th or below 90th percentile based on the amount of likes and retweets in has, assigning a double from 10 to 0.01, repesectively. Percentiles are listed in @fig-1 below. 

```{r}
#| label: fig-1
#| fig-cap: "Tweet Engagement Percentile Thresholds and Assigned Weights"
...
```{r}
#| label: fig-4
#| fig-cap: "*Regression Coefficients for Engagement in Democracy Discourse.* Note: models assume Biden and negative sentiment as a reference."
